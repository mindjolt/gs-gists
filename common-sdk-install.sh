...
function fetch () {
    package=$1
    latest=$(curl --silent -H "X-Result-Detail: info " "https://artifactory.sgn.com:443/artifactory/api/search/artifact?name=${package}.zip&repos=libs-release-local" | jq -r "(.results|=sort_by(.created)[-1].downloadUri).results")
